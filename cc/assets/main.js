...
(function() {
	...
	document.addEventListener("DOMContentLoaded", (event) => {
		...
		const code = urlParams.get('codes')
		console.log(code)
		if (code == null) {
		...
		for (i = 1; i < 6; i++) {
			if (i <= codes.length && codes[i-1] != "") {
				document.getElementById(`text0${i}`).innerHTML = `<strong>${codes[i-1]}</strong>`
			}
			else {
				document.getElementById(`container0${i}`).style.display = 'none';
			}
	...
	// Buttons: buttons01.
		$('#buttons01 > li:nth-child(1) > a').addEventListener(
			'click',
			function(event) { 
				code = document.getElementById("text01").textContent;
	...
	// Buttons: buttons02.
		$('#buttons02 > li:nth-child(1) > a').addEventListener(
			'click',
			function(event) { 
				code = document.getElementById("text02").textContent;
				navigator.clipboard.writeText(code);
			}
		);
	
	// Buttons: buttons03.
		$('#buttons03 > li:nth-child(1) > a').addEventListener(
			'click',
			function(event) { 
				code = document.getElementById("text03").textContent;
				navigator.clipboard.writeText(code);
			}
		);
	
	// Buttons: buttons04.
		$('#buttons04 > li:nth-child(1) > a').addEventListener(
			'click',
			function(event) { 
				code = document.getElementById("text04").textContent;
				navigator.clipboard.writeText(code);
			}
		);
	
	// Buttons: buttons05.
		$('#buttons05 > li:nth-child(1) > a').addEventListener(
			'click',
			function(event) { 
				code = document.getElementById("text05").textContent;
				navigator.clipboard.writeText(code);
			}
		);
